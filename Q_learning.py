...
基于 ε-greedy 策略选择动作，并用 Q-learning 更新规则更新 Q 表。
Q-learning 是离线策略（off-policy）算法：更新时使用下一个状态的最大 Q 值，
而不关心实际执行哪个动作。

更新公式（课本标准写法）：
    Q(S_t, A_t) ← Q(S_t, A_t) + α [R_{t+1} + γ * max_a Q(S_{t+1}, a) - Q(S_t, A_t)]
    其中 max_a Q(S_{t+1}, a) 取 S' 下所有动作的最大 Q 值
...
使用方法：
    agent = QLearning()                           # 创建 Q-learning 智能体
    a = agent.choose_action(state_idx)            # ε-greedy 选动作
    agent.update(s, a, r, s_next, done)           # 更新 Q 表
"""
...
class QLearning:
    ...
    def update(self, state_idx: int, action: int, reward: float,
               next_state_idx, done: bool):
        """
        用 Q-learning 更新规则更新 Q 表。

        公式（实现的就是这个公式）：
            Q(S_t, A_t) ← Q(S_t, A_t) + α [R_{t+1} + γ * max_a Q(S_{t+1}, a) - Q(S_t, A_t)]

        与 SARSA 的关键区别：
          - SARSA 使用 Q(S', A')，其中 A' 是下一步实际执行的动作
          - Q-learning 使用 max_a Q(S', a)，取 S' 下所有动作的最大 Q 值
          - 因此 Q-learning 不需要传入 next_action 参数

        参数:
            state_idx:     当前状态 S（离散索引）
            action:        当前动作 A
            reward:        执行 A 后得到的即时奖励 R
            next_state_idx:下一个状态 S'（离散索引，done 时可为 None）
            done:          本轮是否结束（如果结束，则 max_a Q(S', a) = 0）
        """
        # 当前 Q 值
        ...
        if done:
            ...
            # Q-learning 使用 max_a Q(S',a) —— 下一个状态下的最大 Q 值
